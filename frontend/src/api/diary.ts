...
import type { FirstDayUploadResponse, GenerationResponse } from "@/types/api";
const DEFAULT_TRIP_TITLE = "제목 생성 중...";
...
export async function uploadFirstDayPhotos(
  photos: {
    fileUri: string;
  ...
  formData.append("title", DEFAULT_TRIP_TITLE);
  if (options?.tripId) {
  ...
  for (const photo of photos) {
    console.log(
      "upload 할려는 User ID:",
      useUserStore.getState().userProfile?.userId,
    );
    formData.append(
      "user_id",
      String(useUserStore.getState().userProfile?.userId),
    );
    ...
    if (Platform.OS === "web") {
      const blob = await fetch(photo.fileUri).then((response) =>
        response.blob(),
      );
...
export function fetchTripDayGenerationStatus(tripDayId: number) {
  return request<GenerationResponse>(
    `/trip-days/${tripDayId}/generation-status`,
  );
}
...
export function getTripTitle(TRIP_ID: number) {
  const baseUrl = getApiBaseUrl();
  return new EventSource(`${baseUrl}/trips/${TRIP_ID}/title-stream`);
}
